...
def consider_points(start, start_dir):
    ...
    while proceed:
        stepped_on.append(pos)
        grid_traversed[pos[0]][pos[1]] = ","
        ...
        if pos[0] == end[0] and pos[1] == end[1]:
            ...
            weights[weight_label] = (pos_dir_label(start, start_dir), pos_dir_label(pos, direction), cost_for_leg, list(stepped_on))
            weights["end-" + str(direction)] = (pos_dir_label(pos, direction), "end" , 0, list(stepped_on))
            proceed = False
            continue
        ...
        if count == 1:
            ...
            for this_dir, allowed in enumerate(c_dir):
                ...
                if weight_label not in weights:
                    weights[weight_label] = (pos_dir_label(start, start_dir), pos_dir_label(pos, this_dir), cost_for_leg, list(stepped_on))
                    if allowed:
...
for direction_num, allowed in enumerate(directions):
    if allowed:
        ...
        weights["start-" + pos_dir_label(start, direction_num)] = ("start", pos_dir_label(start, direction_num), cost, [start])
...
for label, val in weights.items():
    print(val)
    graph.add_edge(val[0], val[1], weight=val[2], stepped_on=val[3])
...
min_weight = networkx.path_weight(graph, path, weight="weight")
print("optimal path length", min_weight)

print("seeking all shortest paths...")

paths = networkx.all_shortest_paths(graph, "start", "end", weight="weight", method="dijkstra")
...
all_squares = numpy.sum(grid==".")

for path in pbar:

    for i in range(len(path) - 1):
        if (path[i], path[i+1]) not in seen:
            coords = graph.edges[path[i], path[i+1]]['stepped_on']
            for coord in coords:
                seen_points[coord] = 1
            seen[(path[i], path[i+1])] = 1

    p += 1
    if p % 1000 == 0:
        pbar.set_description(str(len(seen)) + "/" + str(all_edges) + ", " + str(len(seen_points)) + "/" + str(all_squares))
        

print(len(seen_points), "steps involved in the optimal paths")
